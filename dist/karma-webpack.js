...
colors, customFileHandlers, emitter) {
  ...
  applyPlugins.forEach(function (compiler) {
    compiler.hooks.thisCompilation.tap(this.plugin, (compilation, params) => {
      compilation.dependencyFactories.set(EntryDependency, params.normalModuleFactory);
    });
...
Plugin.prototype.make = function (compilation, callback) {
  ...
  async.each(this.files.slice(), (file, callback) => {
    ...
    compilation.addEntry('', dep, name, err => {
      ...
      if (module && module.error && module.error.error && module.error.error.code === 'ENOENT') {
